...
webjs.prototype.pushdata_easy = function(command,data1){
	...
	webjs.prototype.paydata = function(prefix,data, success,fail){	
		tbase.showdebug("pushdata",data);
		data=data.replace(/\?/g,"");
		data=tbase.filteremoji(data);
		var url=window.prefix+ "/"+prefix + "?"+data+"&requretime="+new Date().getTime();
		var me=this;
		//alert(url);
		tbase.showdebug(url);
		$.ajax({
