...
const { Option } = Select;
//const routerUrl = cache.keysMenu.RsDeviceerrorlog;
const routerUrl = '/RsDeviceerrorlog';
...
@Form.create()
@List.create()
export default class RsDeviceerrorlogList extends Component {
  state = {
  ...
  componentDidMount() {
    ...
    dispatch({
      ...
      payload: { dicType: 'errtype' },
      callback: data => {
        //console.log(data);
        this.setState({
          dicerrtype: data.data,
        });
      },
    });
    //设备类型
  ...
  render() {
    ...
    const datadicerrtype = this.state.dicerrtype;
    ...
    const columns = [
      ...
      // { title: '记录id', dataIndex: 'log_id', width: 150, sorter: false },
      {
        ...
        render: (text, record) => {
          if (!datadicerrtype) return <span>{record.error_type}</span>;
          const temp = datadicerrtype.find(item => item.dicCode === record.error_type);
          if (!temp) return <span>{record.error_type}</span>;
      ...
      { title: '处理时长(s)', dataIndex: 'use_time', width: 150, sorter: false },
      { title: '处理说明', dataIndex: 'handle_bak', width: 150, sorter: false },
      { title: '创建时间', dataIndex: 'create_date', width: 150, sorter: false },
    ];

    const listConfig = {
      url: '/api/RsDeviceerrorlog/queryRsDeviceerrorlogList', // 必填,请求url
      scroll: { x: 1950, y: this.state.scrollY }, // 可选配置,同antd table
      rowKey, // 必填,行key
      columns, // 必填,行配置
    };
        ...
        <Card bordered={false} style={{ marginBottom: 24 }} hoverable>
          <Form onSubmit={this.handleSearch}>
            <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
              ...
              <Col {...formItemGrid}>
                {' '}
                <FormItem {...formItemLayout} label="异常类型">
                  {getFieldDecorator('err_type', {
                    initialValue: this.props.list.queryMap.err_type,
                  })(
                    <Select showSearch allowClear placeholder="异常类型">
                      {datadicerrtype &&
                        datadicerrtype.map(item => {
              ...
              <Col {...formItemGrid}>
                {' '}
                <FormItem {...formItemLayout} label="处理时长(s)">
              ...
              <Col {...formItemGrid}>
                ...
                <FormItem {...formItemLayout} label="处理说明">
                  {getFieldDecorator('handle_bak', {
                    initialValue: this.props.list.queryMap.handle_bak,
                  })(<Input placeholder="请输入" />)}{' '}
                </FormItem>{' '}
              </Col>
              <Col {...formItemGrid}>
