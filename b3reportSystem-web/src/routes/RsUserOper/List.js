...
const { Option } = Select;
//const routerUrl = cache.keysMenu.RsUser;
const routerUrl = '/RsUserOper';
const url = 'RsUser';
...
@Form.create()
@List.create()
export default class RsUserList extends Component {
  state = {
  ...
  render() {
    ...
    const columns = [
      ...
      { title: '用户工号', dataIndex: 'user_number', width: 150, sorter: false },
      ...
      // { title: '用户类型', dataIndex: 'dls_user_type', width: 150, sorter: false, },
      ...
      { title: '用户QQ号', dataIndex: 'user_qq', width: 150, sorter: false },
      {
        title: '是否在职',
        // dataIndex: 'is_active',
        width: 150,
    ...
    const listConfig = {
      url: '/api/RsUser/queryRsUserList', // 必填,请求url
      scroll: { x: 1650, y: this.state.scrollY }, // 可选配置,同antd table
      queryMap: { dls_user_type: 'oper' },
      rowKey, // 必填,行key
      columns, // 必填,行配置
    };
        ...
        <Card bordered={false} style={{ marginBottom: 24 }} hoverable>
          <Form onSubmit={this.handleSearch}>
            <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
              ...
              <Col {...formItemGrid}>
                ...
                <FormItem {...formItemLayout} label="用户姓名">
                  {getFieldDecorator('user_name', {
                    initialValue: this.props.list.queryMap.user_name,
                  })(<Input placeholder="请输入" />)}{' '}
                </FormItem>{' '}
              </Col>
              <Col {...formItemGrid}>
                {' '}
                <FormItem {...formItemLayout} label="用户工号">
                  {getFieldDecorator('user_number', {
                    initialValue: this.props.list.queryMap.user_number,
                  })(<Input placeholder="请输入" />)}{' '}
                </FormItem>{' '}
              </Col>
              <Col {...formItemGrid}>
                ...
                <FormItem {...formItemLayout} label="用户手机号">
                  {getFieldDecorator('user_phone', {
                    initialValue: this.props.list.queryMap.user_phone,
                  })(<Input placeholder="请输入" />)}{' '}
                </FormItem>{' '}
              </Col>
              <Col {...formItemGrid}>
              ...
              {/* <Col {...formItemGrid}>  <FormItem {...formItemLayout} label='用户类型'>{getFieldDecorator('dls_user_type', { initialValue: this.props.list.queryMap.dls_user_type, })(<Input placeholder='请输入' />)} </FormItem> </Col> */}
              ...
              <Col {...formItemGrid}>
                ...
                <FormItem {...formItemLayout} label="用户电话">
                  {getFieldDecorator('user_tel', {
                    initialValue: this.props.list.queryMap.user_tel,
                  })(<Input placeholder="请输入" />)}{' '}
                </FormItem>{' '}
              </Col>
              <Col {...formItemGrid}>
                {' '}
                <FormItem {...formItemLayout} label="用户QQ号">
                  {getFieldDecorator('user_qq', { initialValue: this.props.list.queryMap.user_qq })(
                    <Input placeholder="请输入" />
                  )}{' '}
                </FormItem>{' '}
              </Col>
              <Col {...formItemGrid}>
                {' '}
                <FormItem {...formItemLayout} label="是否在职(起始)">
                  {getFieldDecorator('start_is_active', {
                    initialValue: this.props.list.queryMap.start_is_active
                      ? moment(this.props.list.queryMap.start_is_active)
                      : null,
              ...
              <Col {...formItemGrid}>
                {' '}
                <FormItem {...formItemLayout} label="是否在职(结束)">
                  {getFieldDecorator('end_is_active', {
                    initialValue: this.props.list.queryMap.end_is_active
                      ? moment(this.props.list.queryMap.end_is_active)
                      : null,
                  })(<InputNumber placeholder="请输入" />)}{' '}
                </FormItem>{' '}
              </Col>
