 ...
 * * 主要用于：用户设置 接口层，此代码为自动生成
 */
@Controller
@RequestMapping(value = "/api/TsUserSetting")
public class TsUserSettingController {
    @Autowired
    ...
    @ResponseBody
    @Permission(moduleCode = "TsUserSetting",operationCode = "NEW")
    public String addobj(@ModelAttribute TsUserSetting TsUserSetting, OperInfo opUser) throws Exception {
           TsUserSetting obj = TsUserSettingService.addobj(TsUserSetting,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "TsUserSetting",operationCode = "QUERY")
    public String getobj(@RequestParam("id") String id,OperInfo opUser) throws Exception{
        TsUserSetting obj = TsUserSettingService.getobj(id,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "TsUserSetting",operationCode = "UPDATE")
    public String updateobj(@ModelAttribute TsUserSetting TsUserSetting, OperInfo opUser) throws Exception{
        TsUserSetting obj = TsUserSettingService.updateobj(TsUserSetting,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "TsUserSetting",operationCode = "DELETE")
    public String deleteobj(@RequestParam("id") String id,OperInfo opUser) throws Exception {
        TsUserSettingService.deleteobj(id,opUser);
    ...
    public String newObj(OperInfo opUser) throws Exception {
        TsUserSetting obj = TsUserSettingService.newObj(opUser);
        return  TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(), obj);
    }
   
	
    @ApiOperation(value="查询")
	@PostMapping(value = "/queryTsUserSettingList")
    @ResponseBody
    @Permission(moduleCode = "TsUserSetting",operationCode = "QUERY")
    public String  queryTsUserSettingList(@RequestBody Map queryMap, OperInfo operInfo) throws Exception {
		 return TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(),infoQuery.queryTsUserSettingList(queryMap, operInfo));
    }
    ...
    public void exportExcel(@RequestBody Map queryMap, HttpServletResponse response, OperInfo operInfo) throws Exception {
        List list = infoQuery.queryTsUserSettingList(queryMap,operInfo).getList();
        response.reset();
        response.setContentType("application/octet-stream;charset=UTF-8");
        response.setHeader("Pragma", "No-cache");
        OutputStream os = response.getOutputStream();
        ExcelUtil.writeWithMap(list,os,TsUserSetting.class);
        os.close();
    }
    ...
    public String importExcel(@PathVariable("uid")String uid,OperInfo opr) throws Exception {
        ...
        List<TsUserSetting> objs = null;
        if(attachment != null)
        {
            String filePath = attachment.getStorePath() + "/" +  attachment.getNewName() + "."+  attachment.getFileExtension();
            List objList = new ArrayList();
            objs = ExcelUtil.readFrom(filePath,TsUserSetting.class,TsUserSettingService,opr);
            if(objs != null && objs.size() > 0)
            {
                for(TsUserSetting obj : objs)
                {
                    try {
                        String msg = TsUserSettingService.checkAddorUpdate(obj, opr,true);
                ...
                if(objList.size() == 0) {
                    for (TsUserSetting obj : objs) {
                        TsUserSettingService.addobj(obj, opr);
                    }
                }
                else
                {
