...
import com.qgbase.biz.reportSystem.service.RsZoneService;
 ...
 */
@Controller
@RequestMapping(value = "/api/RsZone")
public class RsZoneController {
    @Autowired
    RsZoneService RsZoneService;
	@Autowired
    ...
    @ResponseBody
    @Permission(moduleCode = "RsZone",operationCode = "NEW")
    public String addobj(@ModelAttribute RsZone RsZone, OperInfo opUser) throws Exception {
           RsZone obj = RsZoneService.addobj(RsZone,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "RsZone",operationCode = "QUERY")
    public String getobj(@RequestParam("id") String id,OperInfo opUser) throws Exception{
        RsZone obj = RsZoneService.getobj(id,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "RsZone",operationCode = "UPDATE")
    public String updateobj(@ModelAttribute RsZone RsZone, OperInfo opUser) throws Exception{
        RsZone obj = RsZoneService.updateobj(RsZone,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "RsZone",operationCode = "DELETE")
    public String deleteobj(@RequestParam("id") String id,OperInfo opUser) throws Exception {
        RsZoneService.deleteobj(id,opUser);
    ...
    public String newObj(OperInfo opUser) throws Exception {
        RsZone obj = RsZoneService.newObj(opUser);
        return  TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(), obj);
    }
   
	
    @ApiOperation(value="查询")
	@PostMapping(value = "/queryRsZoneList")
    @ResponseBody
    @Permission(moduleCode = "RsZone",operationCode = "QUERY")
    public String  queryRsZoneList(@RequestBody Map queryMap, OperInfo operInfo) throws Exception {
		 return TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(),infoQuery.queryRsZoneList(queryMap, operInfo));
    }
    ...
    public void exportExcel(@RequestBody Map queryMap, HttpServletResponse response, OperInfo operInfo) throws Exception {
        List list = infoQuery.queryRsZoneList(queryMap,operInfo).getList();
        response.reset();
        response.setContentType("application/octet-stream;charset=UTF-8");
        response.setHeader("Pragma", "No-cache");
        OutputStream os = response.getOutputStream();
        ExcelUtil.writeWithMap(list,os,RsZone.class);
        os.close();
    }
    ...
    public String importExcel(@PathVariable("uid")String uid,OperInfo opr) throws Exception {
        ...
        List<RsZone> objs = null;
        if(attachment != null)
        {
            String filePath = attachment.getStorePath() + "/" +  attachment.getNewName() + "."+  attachment.getFileExtension();
            List objList = new ArrayList();
            objs = ExcelUtil.readFrom(filePath,RsZone.class,RsZoneService,opr);
            if(objs != null && objs.size() > 0)
            {
                for(RsZone obj : objs)
                {
                    try {
                        String msg = RsZoneService.checkAddorUpdate(obj, opr,true);
