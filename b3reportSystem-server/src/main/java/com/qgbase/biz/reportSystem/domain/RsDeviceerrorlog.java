 ...
 * 主要用于：设备异常记录 实体类定义，此代码为自动生成
 */

@Data
@Entity
@Table(name = "rs_deviceerrorlog")
public class RsDeviceerrorlog extends TBaseEntity {
     @Id
...
@NotNull(message = "记录id不能为空")
	...
	@Column(name = "error_type",unique = true,columnDefinition = "varchar(64) COMMENT '异常类型'")
    @ExcelColumn(columnName="异常类型",order = 2,exportName = "error_type")
    private String errorType;//create by mark 异常类型
	...
	@Column(name = "use_time",unique = true,columnDefinition = "bigint COMMENT '处理时长(s)'")
    @ExcelColumn(columnName="处理时长(s)",order = 11,exportName = "use_time")
    private long useTime;//create by mark 处理时长(s)

	@Column(name = "handle_bak",unique = true,columnDefinition = "varchar(512) COMMENT '处理说明'")
    @ExcelColumn(columnName="处理说明",order = 12,exportName = "handle_bak")
    private String handleBak;//create by mark 处理说明
