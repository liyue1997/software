...
import com.qgbase.biz.reportSystem.service.RsUserService;
 ...
 * * 主要用于：代理商用户 接口层，此代码为自动生成
 */
@Controller
@RequestMapping(value = "/api/RsUser")
public class RsUserController {
    ...
    @ResponseBody
    @Permission(moduleCode = "RsUser",operationCode = "NEW")
    public String addobj(@ModelAttribute RsUser RsUser, OperInfo opUser) throws Exception {
           RsUser obj = RsUserService.addobj(RsUser,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "RsUser",operationCode = "QUERY")
    public String getobj(@RequestParam("id") String id,OperInfo opUser) throws Exception{
        RsUser obj = RsUserService.getobj(id,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "RsUser",operationCode = "UPDATE")
    public String updateobj(@ModelAttribute RsUser RsUser, OperInfo opUser) throws Exception{
        RsUser obj = RsUserService.updateobj(RsUser,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "RsUser",operationCode = "DELETE")
    public String deleteobj(@RequestParam("id") String id,OperInfo opUser) throws Exception {
        RsUserService.deleteobj(id,opUser);
    ...
    public String newObj(OperInfo opUser) throws Exception {
        RsUser obj = RsUserService.newObj(opUser);
        return  TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(), obj);
    }
   
	
    @ApiOperation(value="查询")
	@PostMapping(value = "/queryRsUserList")
    @ResponseBody
    @Permission(moduleCode = "RsUser",operationCode = "QUERY")
    public String  queryRsUserList(@RequestBody Map queryMap, OperInfo operInfo) throws Exception {
		 return TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(),infoQuery.queryRsUserList(queryMap, operInfo));
    }
    ...
    public void exportExcel(@RequestBody Map queryMap, HttpServletResponse response, OperInfo operInfo) throws Exception {
        List list = infoQuery.queryRsUserList(queryMap,operInfo).getList();
        response.reset();
        response.setContentType("application/octet-stream;charset=UTF-8");
        response.setHeader("Pragma", "No-cache");
        OutputStream os = response.getOutputStream();
        ExcelUtil.writeWithMap(list,os,RsUser.class);
        os.close();
    }
    ...
    public String importExcel(@PathVariable("uid")String uid,OperInfo opr) throws Exception {
        ...
        List<RsUser> objs = null;
        if(attachment != null)
        {
            String filePath = attachment.getStorePath() + "/" +  attachment.getNewName() + "."+  attachment.getFileExtension();
            List objList = new ArrayList();
            objs = ExcelUtil.readFrom(filePath,RsUser.class,RsUserService,opr);
            if(objs != null && objs.size() > 0)
            {
                for(RsUser obj : objs)
                {
                    try {
                        String msg = RsUserService.checkAddorUpdate(obj, opr,true);
