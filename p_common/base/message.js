define(function(require) {
	...
	var messageParse = function(msg) {
	   // console.log('messageParse',msg);
		var message= IM.messageParse(msg, {
		...
		if (message.content.indexOf('{hongbao:')>=0)
		{
		//alert(message.content);
	    var	hongbaoid=message.content.substring(9,message.content.length-1);
		//alert(hongbaoid);
		//查询红包金额、类型、剩余、图片、口令		
        var t_redtype="";
        var t_redmoney="";
		...
		webjs.pushdata("pay_queryred", "&redid=" + hongbaoid ,function(data) { 
			t_redtype=data.t_redtype;
			t_redmoney=data.t_redmoney;
			t_residuemoney=data.t_residuemoney;
			t_redimg=data.t_redimg;
			t_redtext=data.t_redtext;
			if(t_redtype==="1"){
			    t_redimg="http://fdlx.oss-cn-shanghai.aliyuncs.com/aDTkTR58am.jpg";
		    }
		
		    message.type='hongbao';
		    message.fileUrl=hongbaoid;
		    message.content="<img src='"+t_redimg+"' />";
		    if(t_redtype==="1"){		    
		    	message.messageText="请输入口令："+t_redtext+"<br/>口令红包";
		    }else  if(t_redtype==="2"){
		    	message.messageText="";
		    }
		    ...
		    //message.isOut=false;
		    //message.fileUrl="https://android-artworks.25pp.com/fs08/2017/08/03/9/110_9597b3516a150c5225690ac58ab84edb_con.png";
		   //message.fImg="https://android-artworks.25pp.com/fs08/2017/08/03/9/110_9597b3516a150c5225690ac58ab84edb_con.png";
	...
	Model.prototype.okBtnClick = function(event) {
		if (this.curPeer) {
			...
			if (this.params !== undefined) {
				...
				//判断是否发送口令
				console.log("okBtnClick peer",peer);
				console.log("okBtnClick peer.type",peer.type);
				console.log("okBtnClick text",text);
				var groupid=this.curPeer.id;
				var gettype=this.curPeer.type;
				 //判断是否是在抢口令红包
				 var isok=1;
				 var redid="";				 
			webjs.pushdata("pay_qredistext","&redtext=" + text +"&groupid="+groupid+"&type="+gettype
					+"&senduid="+ttool.getuserid_nocheck()+"&isver="+window.ver,function(data) { 
				if(data.t_redid!==""){
					redid=data.t_redid;
					isok=0;
					IM.sendTextMessage(peer, text);
					justep.Shell.showPage("payredwars" ,{
						"hongbaoid" : redid,
						"groupid" : groupid,
						"gettype" : gettype,
						"redtext" : text
					});
				}else if(data.t_redid===""&&data.info!==""){
													
						ttool.showmsg(data.info); 
						IM.sendTextMessage(peer, text);
	...
	Model.prototype.modelLoad = function(event) {
		...
		self.comp("hbwindowDialog").src=require.toUrl(window.baseurl+"pay/payredpackets.w");
    ...
    Model.prototype.btnhbclick = function(event){
        //console.log(window.baseurl+"pay/payredpackets");
		this.comp("hbwindowDialog").src=require.toUrl(window.baseurl+"pay/payredpackets.w");
		var dialogID=this.curPeer.id;
		var gettype=this.curPeer.type;
		console.log(gettype);
		this.comp("hbwindowDialog").open({data:{dialogID:dialogID,gettype:gettype}});
		//justep.Shell.showPage("payredpackets");
	};
    Model.prototype.btnhbclick1 = function(event){    		
		//console.log("btnhbclick1 event",event);	
		var groupid=this.curPeer.id;	
		var gettype=this.curPeer.type;	
        var row = event.bindingContext.$object.val("fileUrl");
		//console.log("btnhbclick1 event row",row);
		//判断红包是否是口令红包
		webjs.pushdata("pay_isredtext","&redid="+row , function(data) {
		ttool.showmsg("请先发送口令！");
		$(".x-input-content").html(data.redtext);
		}, function(info) {
			justep.Shell.showPage("payredwars" ,{
			"hongbaoid" : row,
			"gettype" : gettype,
	...
	Model.prototype.hbwindowDialogReceive = function(event){
		var redid=event.data.redid;
	//	var redtype=event.data.redtype;//2为普通红包  1为口令红包
	//	var redtext=event.data.redtext;
	//	var redimg=event.data.redimg;
	//	var appendmsg="";
		
		var peer = this.curPeer;
		if (peer) {
			IM.sendTextMessage(peer,"{hongbao:"+redid+"}");						
		}
		this.comp('superinput').closePicker();
		//event.target.value = '';
		
		//$(this.getElementByXid("msgListContent")).append(appendmsg);
    };
	return Model;
});
