define(function(require) {
	...
	var IMImpl = Object.extend({
		...
		logout : function() {
			...
			var m= localStorage.getItem("password");
		...
		getPersonByUID : function(uid) {
			...
			try {
				...
				//console.log("获取用户失败getPersonByUID",uid);
			}// 屏蔽错误
			if (user && persons[user.nick]) {
				...
				//console.log("getPersonByUID",user);
		...
		getPerson : function(id) {
			...
			if (person) {
				...
				try {
					...
					//console.log("获取用户失败getPerson",id);
					//console.log(e);
				}// 屏蔽错误
				if (user) {
			...
			{
				webjs.pushdata("imgetuseronly","&imuid="+id,function(data){
						if (data.sNumb==="")
						{
							console.log("此用户不存在或未注册IM");
						}
						else
						{
							var p = {
								...
								persons[data.sPersonID] = new Person(p);
								person = persons[id];
								console.log("获取用户 web",person);
								if (person) {
									ret = new Person(person);
								}
						}
