	...
	var webjs = function() {
		...
		if (start)
		{
			...
			timer = window.setInterval(function() {
				...
				tbase.showdebug(window.prefix + "command=cll_just&userid="+userid);
				$.ajax({
					"type" : "get",
					"async" : false,
					"cache" : false,
					...
					"error" : function(XMLHttpRequest, textStatus, errorThrown) {
						justep.Util.hint("您貌似断网了，请检查网络,如果联网还不能恢复，可重启软件（You offline ,please check network)", {
							type : 'danger',
							delay : 2000
						});	
					},
			...
			"success" : function(datavalue) {	
				if (datavalue.ret == "success") {
					...
					fail("请求失败："+datavalue.info);
					 if (datavalue.errcode=="zj01")
					 {
							justep.Shell.showPage("zjAuth");
					 }
					 else if (datavalue.errcode=="zj02")
					 {
						 //justep.Shell.showPage("zjAuth");
