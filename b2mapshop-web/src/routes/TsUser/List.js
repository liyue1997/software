...
export default class TsUserList extends Component {
  ...
  componentDidMount() {
    ...
    // const { params } = {dicType:'usertype'}
    //请求用户类型下拉框
    const {dispatch} = this.props
    const {dicDesc} = this.state
    dispatch({
      type: 'base/getByDicType',
      payload: {dicType:'usertype'},
      callback: data => {
        // console.log(data)
        this.setState({
          dicDesc:data
  ...
  render() {
    ...
    const columns = [
      ...
      { 
        ...
        render:(text,record)=>{
          // console.log(record)
          const temp = newdicDesc.find(item=>item.dicCode === record.user_type)
          return(
            <span>{temp.dicValue}</span>
          )
        }
      },
          ...
          <Form onSubmit={this.handleSearch}>
            <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
              <Col {...formItemGrid}> <FormItem {...formItemLayout}
                label='用户id'>{getFieldDecorator('user_id', { initialValue: this.props.list.queryMap.user_id, })(
                  <Input placeholder='请输入' />)} </FormItem> </Col>
              <Col {...formItemGrid}> <FormItem {...formItemLayout}
                label='用户登录账户'>{getFieldDecorator('user_account', { initialValue: this.props.list.queryMap.user_account, })(
                  <Input placeholder='请输入' />)} </FormItem> </Col>
              ...
              <Col {...formItemGrid}> <FormItem {...formItemLayout}
                label='用户名'>{getFieldDecorator('username', { initialValue: this.props.list.queryMap.username, })(
                  <Input placeholder='请输入' />)} </FormItem> </Col>

              <Col {...formItemGrid}> <FormItem {...formItemLayout} label='用户类型'>
                {getFieldDecorator('user_type', { initialValue: this.props.list.queryMap.user_type, })
                  (<Select showSearch allowClear placeholder='用户类型'>
                      {newdicDesc&&newdicDesc.map(item=>{
                        // console.log(item)
                        return (
                          <Option
                        value={item.dicCode}
              ...
              <Col {...formItemGrid}> <FormItem {...formItemLayout}
                label='是否允许登录(起始)'>{getFieldDecorator('start_user_status', { initialValue: this.props.list.queryMap.start_user_status ? moment(this.props.list.queryMap.start_user_status) : null, })
                  (<InputNumber placeholder='请输入' />)} </FormItem> </Col>
              <Col {...formItemGrid}> <FormItem {...formItemLayout}
                label='是否允许登录(结束)'>{getFieldDecorator('end_user_status', { initialValue: this.props.list.queryMap.end_user_status ? moment(this.props.list.queryMap.end_user_status) : null, })
                  (<InputNumber placeholder='请输入' />)} </FormItem> </Col>
              <Col {...formItemGrid}> <FormItem {...formItemLayout}
                label='角色'>{getFieldDecorator('role_id', { initialValue: this.props.list.queryMap.role_id, })(
                  <Input placeholder='请输入' />)} </FormItem> </Col>
              <Col {...formItemGrid}> <FormItem {...formItemLayout}
                label='最后登录ip'>{getFieldDecorator('last_ip', { initialValue: this.props.list.queryMap.last_ip, })(
                  <Input placeholder='请输入' />)} </FormItem> </Col>
              <Col {...formItemGrid}> <FormItem {...formItemLayout}
                label='最后登录系统'>{getFieldDecorator('last_os', { initialValue: this.props.list.queryMap.last_os, })(
                  <Input placeholder='请输入' />)} </FormItem> </Col>
              <Col {...formItemGrid}> <FormItem {...formItemLayout}
                label='最后登录时间(起始)'>{getFieldDecorator('start_last_time', { initialValue: this.props.list.queryMap.start_last_time ? moment(this.props.list.queryMap.start_last_time) : null, })(
                  <DatePicker format={DateFormat} placeholder='请输入' />)} </FormItem> </Col>
              <Col {...formItemGrid}> <FormItem {...formItemLayout}
                label='最后登录时间(结束)'>{getFieldDecorator('end_last_time', { initialValue: this.props.list.queryMap.end_last_time ? moment(this.props.list.queryMap.end_last_time) : null, })(
                  <DatePicker format={DateFormat} placeholder='请输入' />)} </FormItem> </Col>
              <Col {...formItemGrid}> <FormItem {...formItemLayout}
