...
public class TinfoQuery {
     ...
     * 主要用于：查询 数据字典 ，此代码为自动生成
     * @param queryMap 查询条件
     * @return 查询结果
     */
    public PageControl queryTDicList(Map queryMap, OperInfo operInfo) throws Exception {
        //在这里处理各种角色的数据权限，比如 只能看自己的仓库的数据等
        String dataWhere=""; //角色数据条件
        switch (operInfo.getCurrentUser().getRoleId())
        {
        ...
        Map<String,Object> parameter = new HashMap();
        QueryPOB queryPOB = new QueryPOB(queryMap);
        String sql = "select a.dic_id"
                +",a.dic_type,a.dic_code,a.dic_value,a.dic_desc,a.is_used"
                +",a.order_no,a.parent_id,a.pysx,a.dic_data1,a.dic_data2"
                +",a.create_date,a.create_user,a.modify_date,a.modify_user from t_dic a ";
        String whereSql = " where 1=1 "+dataWhere+" ";
        if (null != queryMap && queryMap.size() > 0) {
            if (StringUtil.isNotBlankIfStr(queryMap.get("dic_id"))) {
                whereSql += " and a.dic_id = :dic_id";
                parameter.put("dic_id", queryMap.get("dic_id") );
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("dic_type"))) {
                whereSql += " and a.dic_type = :dic_type";
                parameter.put("dic_type", queryMap.get("dic_type") );
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("dic_code"))) {
                whereSql += " and a.dic_code like :dic_code";
                parameter.put("dic_code", "%" + queryMap.get("dic_code") + "%");
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("dic_value"))) {
                whereSql += " and a.dic_value like :dic_value";
                parameter.put("dic_value", "%" + queryMap.get("dic_value") + "%");
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("dic_desc"))) {
                whereSql += " and a.dic_desc like :dic_desc";
                parameter.put("dic_desc", "%" + queryMap.get("dic_desc") + "%");
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("is_used"))) {
                whereSql += " and a.is_used = :is_used";
                parameter.put("is_used", queryMap.get("is_used") );
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("start_order_no"))) {
                whereSql += " and a.order_no >= :start_order_no ";
                parameter.put("start_order_no",queryMap.get("start_order_no"));
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("end_order_no"))) {
                whereSql += " and a.order_no <= :end_order_no ";
                parameter.put("end_order_no",queryMap.get("end_order_no"));
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("parent_id"))) {
                whereSql += " and a.parent_id = :parent_id";
                parameter.put("parent_id", queryMap.get("parent_id") );
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("pysx"))) {
                whereSql += " and a.pysx like :pysx";
                parameter.put("pysx", "%" + queryMap.get("pysx") + "%");
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("dic_data1"))) {
                whereSql += " and a.dic_data1 like :dic_data1";
                parameter.put("dic_data1", "%" + queryMap.get("dic_data1") + "%");
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("dic_data2"))) {
                whereSql += " and a.dic_data2 like :dic_data2";
                parameter.put("dic_data2", "%" + queryMap.get("dic_data2") + "%");
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("start_create_date"))) {
                whereSql +=" and a.create_date >= str_to_date(:start_create_date,'%Y-%m-%d %H:%i:%s')";
                parameter.put("start_create_date",queryMap.get("start_create_date")+"00:00:00");
    ...
    /**
     * Created by Mark on 2019-08-16
     * 主要用于：查询 用户设置 ，此代码为自动生成
     * @param queryMap 查询条件
     * @return 查询结果
     */
    public PageControl queryTsUserSettingList(Map queryMap, OperInfo operInfo) throws Exception {
        //在这里处理各种角色的数据权限，比如 只能看自己的仓库的数据等
        String dataWhere=""; //角色数据条件
        switch (operInfo.getCurrentUser().getRoleId())
        {
        ...
        Map<String,Object> parameter = new HashMap();
        QueryPOB queryPOB = new QueryPOB(queryMap);
        String sql = "select a.user_setting_id"
                +",a.user_id,a.setting_key,a.setting_value,a.setting_desc,a.create_date"
                +",a.create_user,a.modify_date,a.modify_user from ts_user_setting a ";
        String whereSql = " where 1=1 "+dataWhere+" ";
        if (null != queryMap && queryMap.size() > 0) {
            if (StringUtil.isNotBlankIfStr(queryMap.get("user_setting_id"))) {
                whereSql += " and a.user_setting_id = :user_setting_id";
                parameter.put("user_setting_id", queryMap.get("user_setting_id") );
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("user_id"))) {
                whereSql += " and a.user_id = :user_id";
                parameter.put("user_id", queryMap.get("user_id") );
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("setting_key"))) {
                whereSql += " and a.setting_key like :setting_key";
                parameter.put("setting_key", "%" + queryMap.get("setting_key") + "%");
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("setting_value"))) {
                whereSql += " and a.setting_value like :setting_value";
                parameter.put("setting_value", "%" + queryMap.get("setting_value") + "%");
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("setting_desc"))) {
                whereSql += " and a.setting_desc like :setting_desc";
                parameter.put("setting_desc", "%" + queryMap.get("setting_desc") + "%");
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("start_create_date"))) {
                whereSql +=" and a.create_date >= str_to_date(:start_create_date,'%Y-%m-%d %H:%i:%s')";
                parameter.put("start_create_date",queryMap.get("start_create_date")+"00:00:00");
     ...
     * @return 查询结果
     */
    public PageControl queryImGroupUserList(Map queryMap, OperInfo operInfo) throws Exception {
        //在这里处理各种角色的数据权限，比如 只能看自己的仓库的数据等
        String dataWhere=""; //角色数据条件
        switch (operInfo.getCurrentUser().getRoleId())
        {
        ...
        Map<String,Object> parameter = new HashMap();
        QueryPOB queryPOB = new QueryPOB(queryMap);
        String sql = "select a.group_id"
                +",a.user_id,a.group_title,a.groupstatus,a.create_user,a.create_date"
                +",a.modify_user,a.modify_date from im_group_user a ";
        String whereSql = " where 1=1 "+dataWhere+" ";
        if (null != queryMap && queryMap.size() > 0) {
            if (StringUtil.isNotBlankIfStr(queryMap.get("group_id"))) {
                whereSql += " and a.group_id = :group_id";
                parameter.put("group_id", queryMap.get("group_id") );
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("user_id"))) {
                whereSql += " and a.user_id = :user_id";
                parameter.put("user_id", queryMap.get("user_id") );
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("group_title"))) {
                whereSql += " and a.group_title like :group_title";
                parameter.put("group_title", "%" + queryMap.get("group_title") + "%");
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("start_groupstatus"))) {
                whereSql += " and a.groupstatus >= :start_groupstatus ";
                parameter.put("start_groupstatus",queryMap.get("start_groupstatus"));
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("end_groupstatus"))) {
                whereSql += " and a.groupstatus <= :end_groupstatus ";
                parameter.put("end_groupstatus",queryMap.get("end_groupstatus"));
            }
            if (StringUtil.isNotBlankIfStr(queryMap.get("create_user"))) {
                whereSql += " and a.create_user like :create_user";
                parameter.put("create_user", "%" + queryMap.get("create_user") + "%");
