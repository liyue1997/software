...
import com.qgbase.biz.mapshop.service.SpTeamuserService;
 ...
 * * 主要用于：组团成员 接口层，此代码为自动生成
 */
@Controller
@RequestMapping(value = "/api/SpTeamuser")
public class SpTeamuserController {
    @Autowired
    ...
    @ResponseBody
    @Permission(moduleCode = "SpTeamuser",operationCode = "NEW")
    public String addobj(@ModelAttribute SpTeamuser SpTeamuser, OperInfo opUser) throws Exception {
           SpTeamuser obj = SpTeamuserService.addobj(SpTeamuser,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "SpTeamuser",operationCode = "QUERY")
    public String getobj(@RequestParam("id") String id,OperInfo opUser) throws Exception{
        SpTeamuser obj = SpTeamuserService.getobj(id,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "SpTeamuser",operationCode = "UPDATE")
    public String updateobj(@ModelAttribute SpTeamuser SpTeamuser, OperInfo opUser) throws Exception{
        SpTeamuser obj = SpTeamuserService.updateobj(SpTeamuser,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "SpTeamuser",operationCode = "DELETE")
    public String deleteobj(@RequestParam("id") String id,OperInfo opUser) throws Exception {
        SpTeamuserService.deleteobj(id,opUser);
    ...
    public String newObj(OperInfo opUser) throws Exception {
        SpTeamuser obj = SpTeamuserService.newObj(opUser);
        return  TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(), obj);
    }
   
	
    @ApiOperation(value="查询")
	@PostMapping(value = "/querySpTeamuserList")
    @ResponseBody
    @Permission(moduleCode = "SpTeamuser",operationCode = "QUERY")
    public String  querySpTeamuserList(@RequestBody Map queryMap, OperInfo operInfo) throws Exception {
		 return TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(),mapshopQuery.querySpTeamuserList(queryMap, operInfo));
    }
    ...
    public void exportExcel(@RequestBody Map queryMap, HttpServletResponse response, OperInfo operInfo) throws Exception {
        List list = mapshopQuery.querySpTeamuserList(queryMap,operInfo).getList();
        response.reset();
        response.setContentType("application/octet-stream;charset=UTF-8");
        response.setHeader("Pragma", "No-cache");
        OutputStream os = response.getOutputStream();
        ExcelUtil.writeWithMap(list,os,SpTeamuser.class);
        os.close();
    }
    ...
    public String importExcel(@PathVariable("uid")String uid,OperInfo opr) throws Exception {
        ...
        List<SpTeamuser> objs = null;
        if(attachment != null)
        {
            String filePath = attachment.getStorePath() + "/" +  attachment.getNewName() + "."+  attachment.getFileExtension();
            List objList = new ArrayList();
            objs = ExcelUtil.readFrom(filePath,SpTeamuser.class,SpTeamuserService,opr);
            if(objs != null && objs.size() > 0)
            {
                for(SpTeamuser obj : objs)
                {
                    try {
                        String msg = SpTeamuserService.checkAddorUpdate(obj, opr,true);
                ...
                if(objList.size() == 0) {
                    for (SpTeamuser obj : objs) {
                        SpTeamuserService.addobj(obj, opr);
                    }
                }
                else
                {
