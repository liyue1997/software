...
import com.qgbase.biz.mapshop.service.SpDiscountService;
 ...
 * * 主要用于：商铺优惠 接口层，此代码为自动生成
 */
@Controller
@RequestMapping(value = "/api/SpDiscount")
public class SpDiscountController {
    @Autowired
    ...
    @ResponseBody
    @Permission(moduleCode = "SpDiscount",operationCode = "NEW")
    public String addobj(@ModelAttribute SpDiscount SpDiscount, OperInfo opUser) throws Exception {
           SpDiscount obj = SpDiscountService.addobj(SpDiscount,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "SpDiscount",operationCode = "QUERY")
    public String getobj(@RequestParam("id") String id,OperInfo opUser) throws Exception{
        SpDiscount obj = SpDiscountService.getobj(id,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "SpDiscount",operationCode = "UPDATE")
    public String updateobj(@ModelAttribute SpDiscount SpDiscount, OperInfo opUser) throws Exception{
        SpDiscount obj = SpDiscountService.updateobj(SpDiscount,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "SpDiscount",operationCode = "DELETE")
    public String deleteobj(@RequestParam("id") String id,OperInfo opUser) throws Exception {
        SpDiscountService.deleteobj(id,opUser);
    ...
    public String newObj(OperInfo opUser) throws Exception {
        SpDiscount obj = SpDiscountService.newObj(opUser);
        return  TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(), obj);
    }
   
	
    @ApiOperation(value="查询")
	@PostMapping(value = "/querySpDiscountList")
    @ResponseBody
    @Permission(moduleCode = "SpDiscount",operationCode = "QUERY")
    public String  querySpDiscountList(@RequestBody Map queryMap, OperInfo operInfo) throws Exception {
		 return TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(),mapshopQuery.querySpDiscountList(queryMap, operInfo));
    }
    ...
    public void exportExcel(@RequestBody Map queryMap, HttpServletResponse response, OperInfo operInfo) throws Exception {
        List list = mapshopQuery.querySpDiscountList(queryMap,operInfo).getList();
        response.reset();
        response.setContentType("application/octet-stream;charset=UTF-8");
        response.setHeader("Pragma", "No-cache");
        OutputStream os = response.getOutputStream();
        ExcelUtil.writeWithMap(list,os,SpDiscount.class);
        os.close();
    }
    ...
    public String importExcel(@PathVariable("uid")String uid,OperInfo opr) throws Exception {
        ...
        List<SpDiscount> objs = null;
        if(attachment != null)
        {
            String filePath = attachment.getStorePath() + "/" +  attachment.getNewName() + "."+  attachment.getFileExtension();
            List objList = new ArrayList();
            objs = ExcelUtil.readFrom(filePath,SpDiscount.class,SpDiscountService,opr);
            if(objs != null && objs.size() > 0)
            {
                for(SpDiscount obj : objs)
                {
                    try {
                        String msg = SpDiscountService.checkAddorUpdate(obj, opr,true);
                ...
                if(objList.size() == 0) {
                    for (SpDiscount obj : objs) {
                        SpDiscountService.addobj(obj, opr);
                    }
                }
                else
                {
