...
import com.qgbase.biz.mapshop.service.SpUser2shopService;
 ...
 * * 主要用于：用户浏览收藏得店铺 接口层，此代码为自动生成
 */
@Controller
@RequestMapping(value = "/api/SpUser2shop")
public class SpUser2shopController {
    @Autowired
    ...
    @ResponseBody
    @Permission(moduleCode = "SpUser2shop",operationCode = "NEW")
    public String addobj(@ModelAttribute SpUser2shop SpUser2shop, OperInfo opUser) throws Exception {
           SpUser2shop obj = SpUser2shopService.addobj(SpUser2shop,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "SpUser2shop",operationCode = "QUERY")
    public String getobj(@RequestParam("id") String id,OperInfo opUser) throws Exception{
        SpUser2shop obj = SpUser2shopService.getobj(id,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "SpUser2shop",operationCode = "UPDATE")
    public String updateobj(@ModelAttribute SpUser2shop SpUser2shop, OperInfo opUser) throws Exception{
        SpUser2shop obj = SpUser2shopService.updateobj(SpUser2shop,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "SpUser2shop",operationCode = "DELETE")
    public String deleteobj(@RequestParam("id") String id,OperInfo opUser) throws Exception {
        SpUser2shopService.deleteobj(id,opUser);
    ...
    public String newObj(OperInfo opUser) throws Exception {
        SpUser2shop obj = SpUser2shopService.newObj(opUser);
        return  TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(), obj);
    }
   
	
    @ApiOperation(value="查询")
	@PostMapping(value = "/querySpUser2shopList")
    @ResponseBody
    @Permission(moduleCode = "SpUser2shop",operationCode = "QUERY")
    public String  querySpUser2shopList(@RequestBody Map queryMap, OperInfo operInfo) throws Exception {
		 return TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(),mapshopQuery.querySpUser2shopList(queryMap, operInfo));
    }
    ...
    public void exportExcel(@RequestBody Map queryMap, HttpServletResponse response, OperInfo operInfo) throws Exception {
        List list = mapshopQuery.querySpUser2shopList(queryMap,operInfo).getList();
        response.reset();
        response.setContentType("application/octet-stream;charset=UTF-8");
        response.setHeader("Pragma", "No-cache");
        OutputStream os = response.getOutputStream();
        ExcelUtil.writeWithMap(list,os,SpUser2shop.class);
        os.close();
    }
    ...
    public String importExcel(@PathVariable("uid")String uid,OperInfo opr) throws Exception {
        ...
        List<SpUser2shop> objs = null;
        if(attachment != null)
        {
            String filePath = attachment.getStorePath() + "/" +  attachment.getNewName() + "."+  attachment.getFileExtension();
            List objList = new ArrayList();
            objs = ExcelUtil.readFrom(filePath,SpUser2shop.class,SpUser2shopService,opr);
            if(objs != null && objs.size() > 0)
            {
                for(SpUser2shop obj : objs)
                {
                    try {
                        String msg = SpUser2shopService.checkAddorUpdate(obj, opr,true);
                ...
                if(objList.size() == 0) {
                    for (SpUser2shop obj : objs) {
                        SpUser2shopService.addobj(obj, opr);
                    }
                }
                else
                {
