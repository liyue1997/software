 ...
 * * 主要用于：活动 接口层，此代码为自动生成
 */
@Controller
@RequestMapping(value = "/api/HdHuodong")
public class HdHuodongController {
    @Autowired
    ...
    @ResponseBody
    @Permission(moduleCode = "HdHuodong",operationCode = "NEW")
    public String addobj(@ModelAttribute HdHuodong HdHuodong, OperInfo opUser) throws Exception {
           HdHuodong obj = HdHuodongService.addobj(HdHuodong,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "HdHuodong",operationCode = "QUERY")
    public String getobj(@RequestParam("id") String id,OperInfo opUser) throws Exception{
        HdHuodong obj = HdHuodongService.getobj(id,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "HdHuodong",operationCode = "UPDATE")
    public String updateobj(@ModelAttribute HdHuodong HdHuodong, OperInfo opUser) throws Exception{
        HdHuodong obj = HdHuodongService.updateobj(HdHuodong,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "HdHuodong",operationCode = "DELETE")
    public String deleteobj(@RequestParam("id") String id,OperInfo opUser) throws Exception {
        HdHuodongService.deleteobj(id,opUser);
    ...
    public String newObj(OperInfo opUser) throws Exception {
        HdHuodong obj = HdHuodongService.newObj(opUser);
        return  TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(), obj);
    }
   
	
    @ApiOperation(value="查询")
	@PostMapping(value = "/queryHdHuodongList")
    @ResponseBody
    @Permission(moduleCode = "HdHuodong",operationCode = "QUERY")
    public String  queryHdHuodongList(@RequestBody Map queryMap, OperInfo operInfo) throws Exception {
		 return TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(),huodongQuery.queryHdHuodongList(queryMap, operInfo));
    }
    ...
    public void exportExcel(@RequestBody Map queryMap, HttpServletResponse response, OperInfo operInfo) throws Exception {
        List list = huodongQuery.queryHdHuodongList(queryMap,operInfo).getList();
        response.reset();
        response.setContentType("application/octet-stream;charset=UTF-8");
        response.setHeader("Pragma", "No-cache");
        OutputStream os = response.getOutputStream();
        ExcelUtil.writeWithMap(list,os,HdHuodong.class);
        os.close();
    }
    ...
    public String importExcel(@PathVariable("uid")String uid,OperInfo opr) throws Exception {
        ...
        List<HdHuodong> objs = null;
        if(attachment != null)
        {
            String filePath = attachment.getStorePath() + "/" +  attachment.getNewName() + "."+  attachment.getFileExtension();
            List objList = new ArrayList();
            objs = ExcelUtil.readFrom(filePath,HdHuodong.class,HdHuodongService,opr);
            if(objs != null && objs.size() > 0)
            {
                for(HdHuodong obj : objs)
                {
                    try {
                        String msg = HdHuodongService.checkAddorUpdate(obj, opr,true);
                ...
                if(objList.size() == 0) {
                    for (HdHuodong obj : objs) {
                        HdHuodongService.addobj(obj, opr);
                    }
                }
                else
                {
