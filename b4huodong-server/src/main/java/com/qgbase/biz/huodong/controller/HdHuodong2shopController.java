...
import com.qgbase.biz.huodong.service.HdHuodong2shopService;
 ...
 * * 主要用于：活动门店 接口层，此代码为自动生成
 */
@Controller
@RequestMapping(value = "/api/HdHuodong2shop")
public class HdHuodong2shopController {
    @Autowired
    HdHuodong2shopService HdHuodong2shopService;
	@Autowired
    ...
    @ResponseBody
    @Permission(moduleCode = "HdHuodong2shop",operationCode = "NEW")
    public String addobj(@ModelAttribute HdHuodong2shop HdHuodong2shop, OperInfo opUser) throws Exception {
           HdHuodong2shop obj = HdHuodong2shopService.addobj(HdHuodong2shop,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "HdHuodong2shop",operationCode = "QUERY")
    public String getobj(@RequestParam("id") String id,OperInfo opUser) throws Exception{
        HdHuodong2shop obj = HdHuodong2shopService.getobj(id,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "HdHuodong2shop",operationCode = "UPDATE")
    public String updateobj(@ModelAttribute HdHuodong2shop HdHuodong2shop, OperInfo opUser) throws Exception{
        HdHuodong2shop obj = HdHuodong2shopService.updateobj(HdHuodong2shop,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "HdHuodong2shop",operationCode = "DELETE")
    public String deleteobj(@RequestParam("id") String id,OperInfo opUser) throws Exception {
        HdHuodong2shopService.deleteobj(id,opUser);
    ...
    public String newObj(OperInfo opUser) throws Exception {
        HdHuodong2shop obj = HdHuodong2shopService.newObj(opUser);
        return  TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(), obj);
    }
   
	
    @ApiOperation(value="查询")
	@PostMapping(value = "/queryHdHuodong2shopList")
    @ResponseBody
    @Permission(moduleCode = "HdHuodong2shop",operationCode = "QUERY")
    public String  queryHdHuodong2shopList(@RequestBody Map queryMap, OperInfo operInfo) throws Exception {
		 return TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(),huodongQuery.queryHdHuodong2shopList(queryMap, operInfo));
    }
    ...
    public void exportExcel(@RequestBody Map queryMap, HttpServletResponse response, OperInfo operInfo) throws Exception {
        List list = huodongQuery.queryHdHuodong2shopList(queryMap,operInfo).getList();
        response.reset();
        response.setContentType("application/octet-stream;charset=UTF-8");
        response.setHeader("Pragma", "No-cache");
        OutputStream os = response.getOutputStream();
        ExcelUtil.writeWithMap(list,os,HdHuodong2shop.class);
        os.close();
    }
    ...
    public String importExcel(@PathVariable("uid")String uid,OperInfo opr) throws Exception {
        ...
        List<HdHuodong2shop> objs = null;
        if(attachment != null)
        {
            String filePath = attachment.getStorePath() + "/" +  attachment.getNewName() + "."+  attachment.getFileExtension();
            List objList = new ArrayList();
            objs = ExcelUtil.readFrom(filePath,HdHuodong2shop.class,HdHuodong2shopService,opr);
            if(objs != null && objs.size() > 0)
            {
                for(HdHuodong2shop obj : objs)
                {
                    try {
                        String msg = HdHuodong2shopService.checkAddorUpdate(obj, opr,true);
                ...
                if(objList.size() == 0) {
                    for (HdHuodong2shop obj : objs) {
                        HdHuodong2shopService.addobj(obj, opr);
                    }
                }
                else
                {
