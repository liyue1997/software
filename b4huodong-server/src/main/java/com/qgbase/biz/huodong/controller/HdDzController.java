...
import com.qgbase.biz.huodong.service.HdDzService;
 ...
 * * 主要用于：对账单 接口层，此代码为自动生成
 */
@Controller
@RequestMapping(value = "/api/HdDz")
public class HdDzController {
    @Autowired
    HdDzService HdDzService;
	@Autowired
    ...
    @ResponseBody
    @Permission(moduleCode = "HdDz",operationCode = "NEW")
    public String addobj(@ModelAttribute HdDz HdDz, OperInfo opUser) throws Exception {
           HdDz obj = HdDzService.addobj(HdDz,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "HdDz",operationCode = "QUERY")
    public String getobj(@RequestParam("id") String id,OperInfo opUser) throws Exception{
        HdDz obj = HdDzService.getobj(id,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "HdDz",operationCode = "UPDATE")
    public String updateobj(@ModelAttribute HdDz HdDz, OperInfo opUser) throws Exception{
        HdDz obj = HdDzService.updateobj(HdDz,opUser);
    ...
    public String newObj(OperInfo opUser) throws Exception {
        HdDz obj = HdDzService.newObj(opUser);
        return  TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(), obj);
    }
   
	
    @ApiOperation(value="查询")
	@PostMapping(value = "/queryHdDzList")
    @ResponseBody
    @Permission(moduleCode = "HdDz",operationCode = "QUERY")
    public String  queryHdDzList(@RequestBody Map queryMap, OperInfo operInfo) throws Exception {
		 return TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(),huodongQuery.queryHdDzList(queryMap, operInfo));
    }
    ...
    public void exportExcel(@RequestBody Map queryMap, HttpServletResponse response, OperInfo operInfo) throws Exception {
        List list = huodongQuery.queryHdDzList(queryMap,operInfo).getList();
        response.reset();
        response.setContentType("application/octet-stream;charset=UTF-8");
        response.setHeader("Pragma", "No-cache");
        OutputStream os = response.getOutputStream();
        ExcelUtil.writeWithMap(list,os,HdDz.class);
        os.close();
    }
    ...
    public String importExcel(@PathVariable("uid")String uid,OperInfo opr) throws Exception {
        ...
        if(attachment != null)
        {
            ...
            objs = ExcelUtil.readFrom(filePath,HdDz.class,HdDzService,opr);
            if(objs != null && objs.size() > 0)
            {
                for(HdDz obj : objs)
                {
                    try {
                        String msg = HdDzService.checkAddorUpdate(obj, opr,true);
