...
import com.qgbase.biz.huodong.domain.HdZone;
import com.qgbase.biz.huodong.service.HdZoneService;
 ...
 * * 主要用于：区域 接口层，此代码为自动生成
 */
@Controller
@RequestMapping(value = "/api/HdZone")
public class HdZoneController {
    @Autowired
    HdZoneService HdZoneService;
	@Autowired
    ...
    @ResponseBody
    @Permission(moduleCode = "HdZone",operationCode = "NEW")
    public String addobj(@ModelAttribute HdZone HdZone, OperInfo opUser) throws Exception {
           HdZone obj = HdZoneService.addobj(HdZone,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "HdZone",operationCode = "QUERY")
    public String getobj(@RequestParam("id") String id,OperInfo opUser) throws Exception{
        HdZone obj = HdZoneService.getobj(id,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "HdZone",operationCode = "UPDATE")
    public String updateobj(@ModelAttribute HdZone HdZone, OperInfo opUser) throws Exception{
        HdZone obj = HdZoneService.updateobj(HdZone,opUser);
    ...
    @ResponseBody
    @Permission(moduleCode = "HdZone",operationCode = "DELETE")
    public String deleteobj(@RequestParam("id") String id,OperInfo opUser) throws Exception {
        HdZoneService.deleteobj(id,opUser);
    ...
    public String newObj(OperInfo opUser) throws Exception {
        HdZone obj = HdZoneService.newObj(opUser);
        return  TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(), obj);
    }
   
	
    @ApiOperation(value="查询")
	@PostMapping(value = "/queryHdZoneList")
    @ResponseBody
    @Permission(moduleCode = "HdZone",operationCode = "QUERY")
    public String  queryHdZoneList(@RequestBody Map queryMap, OperInfo operInfo) throws Exception {
		 return TToolRequest.Result(EnumResultType.SUCCESS.getCode(), EnumResultType.SUCCESS.getMsg(),huodongQuery.queryHdZoneList(queryMap, operInfo));
    }
    ...
    public void exportExcel(@RequestBody Map queryMap, HttpServletResponse response, OperInfo operInfo) throws Exception {
        List list = huodongQuery.queryHdZoneList(queryMap,operInfo).getList();
        response.reset();
        response.setContentType("application/octet-stream;charset=UTF-8");
        response.setHeader("Pragma", "No-cache");
        OutputStream os = response.getOutputStream();
        ExcelUtil.writeWithMap(list,os,HdZone.class);
        os.close();
    }
    ...
    public String importExcel(@PathVariable("uid")String uid,OperInfo opr) throws Exception {
        ...
        List<HdZone> objs = null;
        if(attachment != null)
        {
            String filePath = attachment.getStorePath() + "/" +  attachment.getNewName() + "."+  attachment.getFileExtension();
            List objList = new ArrayList();
            objs = ExcelUtil.readFrom(filePath,HdZone.class,HdZoneService,opr);
            if(objs != null && objs.size() > 0)
            {
                for(HdZone obj : objs)
                {
                    try {
                        String msg = HdZoneService.checkAddorUpdate(obj, opr,true);
