...
public class TUserService extends TBaseBo<TUser> {
     ...
     * @throws Exception
     */
    public void restPassword(String id,String password, OperInfo oper) throws Exception {
        TUser tuser = super.getobj(id, oper);
        if (!checkOpRole("restPassword", tuser, oper)) {
            throw new SysRunException("-2", "没有权限", id);
        }
        //String password = MD5.MD5Encode(password);
    ...
    public TUser login(String userName, String pwd, OperInfo oper) throws Exception {
        ...
        if (oldp.equals(pwd)) {
            ...
            if (user.getUserType().equals("dls"))
            {
                Map userinfos = commonDao.getSqlMap("select d.dls_id,d.dls_name,z.zone_code,z.zone_name,z.zone_lon,z.zone_lat,z.zone_level" +
                        " from rs_user u" +
                        " join rs_dls d on u.dls_id=d.dls_id" +
                        " LEFT JOIN rs_zone z on d.zone_code=z.zone_code" +
                        " where u.user_id=?", user.getUserId());

                extend.put("dls_id",userinfos.get("dls_id"));
                extend.put("dls_name",userinfos.get("dls_name"));
                extend.put("zonecode",userinfos.get("zone_code"));
                extend.put("zonename",userinfos.get("zone_name"));
                extend.put("zonelon",userinfos.get("zone_lon"));
                extend.put("zonelan",userinfos.get("zone_lat"));
                extend.put("zonelevel",userinfos.get("zone_level"));
            }
            if (extend.get("zonecode")==null) {
    ...
    public void forgetpwd(String useraccount, String password, OperInfo oper)throws Exception {
        TUser user =getUserByUserAccount(useraccount);

        restPassword(user.getUserId(),password,oper);
    }
}
