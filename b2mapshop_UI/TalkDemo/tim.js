define(function(require) {
    var TIM = require("$UI/b2mapshop_UI/TIM/tim-js");
    var COS = require("$UI/b2mapshop_UI/TIM/cos-js-sdk-v5");
    ...
    timtalk.prototype.getUserinfo=function(userid,success,info){
    	//console.log("tim getUserinfo",userid);
    	var promise = tim.getUserProfile({
    		  userIDList: [userid] //即使只拉取一个用户的资料，也需要用数组类型，例如：userIDList: ['user1']
    		});
    		promise.then(function(imResponse) {
    			success(imResponse.data,info);
    		}).catch(function(imError) {
    		  console.log('getUserProfile error:', imError);
    		});
    };
    ...
    timtalk.prototype.getGroupinfo=function(pid,success){
    	...
    	promise.then(function(imResponse) {
    	  ...
    	  console.log('getGroupProfile error:', imError);
    	});
    };
...
//		       var msg={id:i,msgtype:msgtype,msgcontent:msgcontent,fromwho:fromwho};
	...
	timtalk.prototype.login=function(userid){
		   ...
		   var userSig=null;
	       var options = {
			  SDKAppID: 1400259159 //您的云通信应用的 SDKAppID
			};
			...
			*/
			...
			tim.on(TIM.EVENT.KICKED_OUT, function(event) {
			  ...
			  // event.data.type - 被踢下线的原因，例如 TIM.TYPES.KICKED_OUT_MULT_ACCOUNT 多账号登录被踢
				...
				if(answer){
					justep.Shell.closePage("TIMmessage");
			...
			webjs.pushdata("app/auth","timsign","&userid=" +userid+"",function(data){
					userSig=data.info;
					// 开始登录
					var nowid=localStorage.getItem("inputuserid");
					tim.login({userID:nowid, userSig: userSig});
			        //justep.Shell.showPage("TIMlist",{id:nowid});
    ...
    timtalk.prototype.sendMessage =function(text,pid,type,success,fail){
		...
		promise.then(function(imResponse) {
		  ...
		  tbase.showmsg("发送失败："+imError.message);
		});
 ...
 // 打开某个会话时，下一次拉取消息列表
    ...
    timtalk.prototype.getTalklist=function(success){
    	...
    	promise.then(function(imResponse) {
    	  ...
    	  tbase.showmsg("加载失败："+imError.message);
    	});
    };
