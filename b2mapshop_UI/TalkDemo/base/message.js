define(function(require) {
    ...
    {	
            ...
            if(type==="user"){
	        	 TIM.getUserinfo(pid,function(data,info){
	        	    thisform.puser=data;
       ...
       var avatar=null;
         ...
         if(message.elements[0].type==="TIMTextElem"){
            ...
            console.log("TIMImageElem");
        	 ...
        	 //如果是音频文件  声音格式要补全
        	 if(voicetype==="wav"||voicetype==="mp3"){
	        	 msgtype='audio';
        	 ...
        	 else if(voicetype==="mp4"||voicetype==="rmvb"||voicetype==="avi"||voicetype==="flv"||voicetype==="mov"||voicetype==="mkv"||voicetype==="m4v"||voicetype==="avi"||voicetype==="mpg"){
        		 msgtype='video';
	        	 content='<div id="videoplayer"><video controls  class="videoplay video">'+'<source src="'+fileurl+'">'+'</video></div>';
        	 }
        	 else{
        		 msgtype='document';
...
//		}
	...
	Model.prototype.imgChange = function(event){
		var type= thisform.curPeer.type;
		var id = thisform.curPeer.id; //自己的id
		var pid = thisform.curPeer.pid;//对方的id
		if (!event.target.files) {
		...
		if (file.length<=0) return;
			...
			TIM.sendimg(id,pid,type,document.getElementById('imgPicker'),function(imResponse){
				console.log("imgupload:",imResponse);
				var message=imResponse.data.message;
				thisform.setMessages([message],0);
			});
	...
	Model.prototype.videoChange = function(event){
		var type= thisform.curPeer.type;
		var id = thisform.curPeer.id; //自己的id
		var pid = thisform.curPeer.pid;//对方的id
		if (!event.target.files) {
		...
		if (file.length<=0) return;
			...
			TIM.sendfile(id,pid,type,document.getElementById('videoPicker'),function(imResponse){
				console.log("videoupload:",imResponse);
				var message=imResponse.data.message;
				thisform.setMessages([message],0);
			});
	};
	...
	Model.prototype.button8Click = function(event){
		document.getElementById('voicePicker').click();
	};
	Model.prototype.voiceChange = function(event){
		var type= thisform.curPeer.type;
		var id = thisform.curPeer.id; //自己的id
		var pid = thisform.curPeer.pid;//对方的id
		if (!event.target.files) {
		...
		if (file.length<=0) return;
			...
			TIM.sendfile(id,pid,type,document.getElementById('voicePicker'),function(imResponse){
				console.log("voiceupload:",imResponse);
				var message=imResponse.data.message;
				thisform.setMessages([message],0);
			});
	};
	
	Model.prototype.showinscorl = function(event){
			document.querySelector('#superinputArea').scrollIntoView();
	};
	return Model;
